...
def writing_text_files():
    ...
    if file:
        if WORK_FOLDER_PATH in file:
            ...
            Button(f2, text='ОТМЕНА', command=f2.destroy).grid(row=2, column=2)
            f2.mainloop()
        else:
...
def rename_file():
    ...
    if file:
        if WORK_FOLDER_PATH in file:
            ...
            Button(f1, text='ОТМЕНА', command=f1.destroy).grid(row=2, column=2, padx=10, pady=10)
            f1.mainloop()
        else:
            messagebox.showinfo('Ошибка', "Выход за пределы рабочей папки")
...
def rename_folder():
    ...
    if folder_name != WORK_FOLDER_PATH:
        ...
        Button(f1, text='ОТМЕНА', command=f1.destroy).grid(row=2, column=2)
        f1.mainloop()
    else:
        messagebox.showinfo('ОШИбКА', "Выход за пределы рабочей папки")
...
def view_folder():
    ...
    exit_button = Button(f1, text='OК', bg='dark green', fg='white', font="bold", command=f1.destroy)
